 ...
 * Использование: npx ts-node scripts/check-worker-schedule.ts worker-2
 */
  ...
  try {
    ...
    if (!result.Items || result.Items.length === 0) {
      ...
      if (allWaiting.Items && allWaiting.Items.length > 0) {
        ...
        allWaiting.Items.forEach((job: any, idx: number) => {
          console.log(
    ...
    // Группируем по runId
    const byRun: { [key: string]: any[] } = {};
    result.Items.forEach((job: any) => {
    ...
    // Выводим по runId
    Object.entries(byRun).forEach(([runId, jobs]: [string, any[]]) => {
      console.log(`\n📦 Run: ${runId}`);
      console.log('-'.repeat(80));
